...
impl Transaction {
    ...
    pub fn extract(&self) -> (Account, Account, Tokens128, Tokens128) {
        match &self.operation {
            Operation::Mint { from, to, amount } => (*from, to.clone(), *amount, Tokens128::ZERO),
            Operation::Transfer {
                from,
                to,
                amount,
            ...
            Operation::Burn { from, amount } => {
                (from.clone(), from.clone(), *amount, Tokens128::ZERO)
            }
            Operation::Auction { to, amount } => (to.clone(), to.clone(), *amount, Tokens128::ZERO),
            Operation::Claim { from, to, amount } => {
                (from.clone(), to.clone(), *amount, Tokens128::ZERO)
            }
        }
    }
}

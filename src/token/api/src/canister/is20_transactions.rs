...
//     transfers: Vec<(TokenReceiver, Tokens128)>,
...
//                 .expect("never fails due to checks above");
//             transfer_balance(balances, from, TokenHolder::from(to), value).expect("never fails due to checks above");
//         }
//     }
//
...
mod tests {
    ...
    //         state.stats.fee_to = TokenHolder::from(john());
    //         drop(state);
    //         assert_eq!(Tokens128::from(1000), canister.balanceOf(alice()));

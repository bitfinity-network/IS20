...
async fn transfer_and_notify(to: Principal, amount: Nat, fee_limit: Option<Nat>) -> TxReceipt {
    let id = transfer(to, amount, fee_limit)?;
    notify(id).await
}
...
mod tests {
    ...
    async fn notify_transaction() {
        ...
        let id = transfer(bob(), Nat::from(AMOUNT), None).unwrap();
    ...
    async fn double_notification() {
        MockContext::new()
            ...
            .inject();
        default_canister_init();
        let id = transfer(bob(), Nat::from(100), None).unwrap();
    ...
    async fn transfer_and_notify_success() {
        ...
        default_canister_init();
        let id = transfer_and_notify(bob(), Nat::from(100), None)
            .await
            .unwrap();
        assert!(*is_notify_called.borrow());

...
pub fn transfer_from(
    ...
    match allowances.get(&caller.from(), &caller.inner()) {
        Some(allowance) => {
            let allowance_new =
                (allowance - value_with_fee).expect("allowance sufficiency checked above");

            if allowance_new != Tokens128::ZERO {
                allowances
                    .insert(&caller.from(), &caller.inner(), allowance_new)
                    .unwrap_or_else(|e| {
...
pub fn approve(
    ...
    if amount_with_fee == Tokens128::from(0u128) {
        ...
        state
            .allowances
            .insert(&caller.inner(), &caller.recipient(), amount_with_fee)
            .unwrap_or_else(|e| {
                ic_canister::ic_kit::ic::trap(&format!("failed to serialize key: {}", e))
            });
    }
...
fn burn(
...
) -> TxReceipt {
    {
        ...
        if new_balance == Tokens128::ZERO {
            ...
            state.balances.insert(from, new_balance);
        }
    }
    state.stats.total_supply =
...
pub(crate) fn transfer_balance(
    ...
    let from_balance_new = (from_balance - amount).ok_or(TxError::InsufficientBalance)?;
    if from_balance_new != Tokens128::ZERO {
    ...
    let to_balance_new = (to_balance + amount).expect(

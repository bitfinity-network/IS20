...
The InfinitySwap (IS20) token standard is a fungible token standard for Internete Compture (IC). This standandard is an
extention of ICRC-1 standard and also includes main methods from Ethereum ERC20 token standard.

Key points:
* compatible with ICRC-1 standard
* ERC20 methods `mint`, `burn` and `transfer`
* batch transfers
* possibility to provide user-claimable rewards
* cycle auctions
...
Subaccounts are identified by an arbirtrary 32-byte byte array. The subaccount with all bytes set to 0 is a `default
subaccount`. Whenever a subaccount is not specified for an operation, the default subaccount will be used.

So an account is fully speciefied by a `Principal` and `Subaccount`:
...
The ICRC-1 stndard must be fully supported. Refer to the standard specification for detailed description of the methods:
...
2. ICRC-1 standard uses single method for minting, burning and transferring tokens. This is done by appinting a special
...
* `holder: opt Principal` - the principal to remove the tokens from. If not specified, the tokens are removed from the
  caller's account. Only token canister owner can burn tokens of other principal. All other accounts can burn only own
  tokens. In case a burn requested for other principal's account and the caller is not the owner,
...
Sometimes we want to allow another user to receive our tokens by request. ERC20 standard provides 'approve` and
`transferFrom` methods for that. IS20 tokens may include `claim` which provides similar functionality.

Tokens holder can transfer his tokens on his subaccount with an ID calculated from claimer `Principal`. If that
...
The subaccount id to send the tokens to to make them claimable is calculated by the following algorithm:
...
Since IC uses reverse gas model to pay for incomming requests, it's important for token canisters to provide a way to
...
Auction is performed in regular intervals set by the `set_auction_period` method. At the beginning of the auction period
...
Arguments:
* `principal: opt Principal` - if set, only the transactions in which the given principal participated are retruned
...
* `count: nat64` - number of transactions to return. The canister can choose to limit maximum number it would return. In
  case this argument value exceeds this limit, max number of transactions is returned.

...
ERC20 is a well established and widely used token standard in Etherium ecosystem. Internet Computer
...
The second issue is addressed with the transaction notifications.
...
If the fee is configured, the user `to` receives `value - fee` tokens. In this case sending
less than `fee amount` is allowed to trap.

```
update transfer(to: Principal, value: Nat) : TxReceipt
```

...
The method `getSignedTransaction` can be used to retrieve a transaction signed by the token canister and IC node.

```
query getSignedTransaction(tx_id: nat) -> SignedTx;

type SignedTx = record {

...
dfx deploy --no-wallet --network ${NETWORK} --argument "(principal \"$CONTROLLER_PRINCIPAL\", null)" token_factory
dfx canister --no-wallet --network ${NETWORK} call token_factory upgrade
dfx deploy --no-wallet --network ${NETWORK} --argument 'record {logo = ""; name = "y"; symbol = "y"; decimals = 8; totalSupply = 1000000000; owner = principal "aaaaa-aa"; fee = 0; feeTo = principal "aaaaa-aa";}' token
